...
# #####################################################
#2016-10-29
想实现异步消息订阅，发现网上大部分教程都需要引入libevent.
仅仅为了这么一个小功能就引入这么一个大库不合适，所以打算用redis自家的libae

由于hiredis没有引入ae的实现,所以需要从src下复制过来(复制下面文件到当前目录)
   config.h
   ae.c
   ae.h
   ae_epoll.c
修改Makefile， 在OBJ加入ae.o
注释掉BINS
直接make or make static 生成hiredis库
# #####################################################
